...
# will also cimport data types
from .ThostFtdcUserApiStruct cimport *

# expose cpp class we wrote
...
cdef extern from "ThostFtdcTraderApi.h":
    cdef cppclass CTraderApi "CThostFtdcTraderApi":  # rename
        # 获取API的版本信息
        # @retrun 获取到的版本号
        @ staticmethod
        ...
        # nogil keyword, Cython will check no interaction with python
        ...
        # replace with the class we wrote
        # 注册回调接口
        ...
        # 用户口令更新请求2
        int ReqUserPasswordUpdate2(CThostFtdcUserPasswordUpdateField *pUserPasswordUpdate,
                                   int nRequestID) nogil except +
...
cdef extern from "ThostFtdcTraderApi.h" namespace "CThostFtdcTraderApi":
    # catch C++ exception if any
    # nogil for python threads
